...
\begin{itemize}
    ...
    \item \textbf{Estimations et charges} : estimation initiale (\texttt{timeoriginalestimate}), temps restant (\texttt{timeestimate}), temps passé (\texttt{timespent}), et Story Points (champ personnalisé \texttt{customfield\_10016}) pour les calculs de vélocité et les simulations Monte Carlo.
    \item \textbf{Sprints} : champ personnalisé (ex. \texttt{customfield\_10020}) contenant les informations de sprint (nom, dates de début et de fin), permettant de regrouper les tickets pour l’analyse de vélocité historique.
